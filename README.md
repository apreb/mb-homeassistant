...
| Inverter AC current (A, signed) | Output AC current of the inverter | with the battery level |
| Home battery send interval | Repeat rate for the two above (1-60s, default 5s) | — |
| Log every value sent | Troubleshooting | — |

Ideal send interval when an inverter current sensor is configured is 1 - 5s, however don't set it lower thant the original HA sensor polling rate. 30-60 s is fine for battery level alone; the device drops the data after 60s with no data.
...
The current is **signed**, and the sign is load-bearing: positive means the inverter is injecting, negative means it is charging the battery. Don't feed it an absolute value.
